...
- 编辑器设置面板已有 World Diagnostics 页，可查看 generation graph、rebuild executor、local scope、预算、source 资产健康、pack integrity、cooked source stale、rebuild plan 命令、受控 dry-run/cook 执行结果、partition streaming、runtime payload 和可见对象/植被统计；Objects 页已支持 archetype 选择、地形拾取放置、删除、undo/redo 和对象 sidecar 保存。
...
当前最重要的方向是把这些能力从“已有受控局部 cook 执行入口”推进到“可恢复、可批量调度、可预算化的非破坏性内容生产系统”。
...
因此当前项目已经具备业界级开放世界管线的关键底座：可校验 source/cooked 分层、world partition、content-addressed package、generation graph、局部 rebuild plan、scoped cook 和编辑器内受控 dry-run/cook 执行。距离最终“业界最佳编辑体验”还差失败恢复与批量调度、真实 spline 工具、可视化 collision/nav 调试、预算化渲染闭环和更高质量美术内容填充。
...
- 在已有 World Diagnostics 基础上继续增强 region/cell 可视化、streaming debug、LOD debug、保存状态、地图统计、cooked stale 诊断、rebuild command plan 和受控 cook 执行结果。
- 在已具备受控 dry-run/cook 执行的基础上，补批量重建、区域锁定、局部重新生成、灾难恢复提示和验证结果跳转。
...
1. 强化 World Diagnostics 执行闭环：批量重建、区域锁定、失败定位、恢复提示、验证结果跳转和执行历史。
