---
name: network
description: "在处理网络连接诊断与代理排障时使用。USE WHEN: 请求失败、主机不可达、DNS、TLS、代理、系统代理、WinHTTP、路由、网络环境问题、可用代理判断。KEYWORDS: network, dns, tls, proxy, system proxy, winhttp, http, socks, vpn, connectivity, timeout, git, npm, 网络, 代理, 系统代理, DNS, TLS, 连通性"
---
...
排障时不能只看一种来源。环境变量、`git` / `npm` / `pnpm` 等工具的本地配置，以及系统级代理都可能单独影响结果。**必须主动检查系统代理**，尤其是用户没有明确提到代理时也要查；很多问题来自浏览器、VS Code、Git、Node、PowerShell 使用的代理来源不一致。
...
只要错误涉及外网、GitHub、npm、模型 API、下载、TLS、timeout、connection reset、认证跳转或浏览器能访问但 CLI 失败，就必须检查系统代理。系统代理可能来自 Windows 设置、PAC、VPN、公司网络、安全软件、TUN/透明代理或 VS Code 继承的环境；即使 `http_proxy` 和工具级代理为空，也不能据此认为“没有代理”。

```powershell
# Windows 系统代理和 WinHTTP 必查
...
Windows 上经常出现浏览器能走系统代理，但 CLI 仍然失败，或者 WinHTTP 和系统代理不一致。涉及命令行工具、安装器、脚本或服务时，必须把 Windows 用户系统代理和 WinHTTP 代理都检查一遍，并明确说明它们是否为空、是否指向同一个代理、是否使用 PAC。
...
3. 代理相关时，是否同时检查了环境变量、工具级配置、系统代理/PAC 和 Windows 下的 WinHTTP。
