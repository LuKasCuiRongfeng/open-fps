...
async function main() {
  ...
  for (const preset of context.presets) {
    ...
    const objects = await generateWorldObjectAssets(context, preset);
    results.push({ preset, terrain, paint, vegetation, objects, cooked: null });
  }
  ...
  for (const result of results) {
    ...
    console.log(`  World objects: ${result.objects.objectCount} in ${result.objects.cellCount} partition cells`);
